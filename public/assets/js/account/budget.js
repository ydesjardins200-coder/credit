/**
 * Budget page — standalone page boot.
 *
 * Fourth (and largest) per-tab page extracted from account.html (Phase
 * D-3c of the account-architecture refactor; see
 * docs/account-architecture.md).
 *
 * Education (Phase C) was the smoke test (no JS). Profile (Phase D-1b)
 * was the real test of JS-bearing extraction (~530 lines). Offers
 * (Phase D-2b) confirmed the pattern (no JS again). Budget is the
 * final extraction at scale: ~4,000 lines of JS spanning multiple
 * subsystems. If anything in the per-tab architecture was going to
 * break, it would break here.
 *
 * Subsystems contained:
 *   - Budget tab read path (initBudgetTab, refreshBudgetTab)
 *   - Renderers: summary, opening/closing balance, upgrade pitch,
 *     bank placeholder, categories, spending headline, calendar
 *     heatmap, goals, entries
 *   - Add Entry modal (Phase 3)
 *   - Add Goal modal (Phase 5c)
 *   - All Entries modal (Phase 5g)
 *   - Opening Balance modal (Phase 5j)
 *   - CSV Import modal (Phase 5d) — multi-step wizard with parsing,
 *     mapping, dedup, review, commit
 *   - Manage Categories view (Phase 4) — edit/archive/reorder
 *   - Multi-month navigation (Phase 5b)
 *
 * Lib dependencies (loaded by budget.html before this file):
 *   - lib/locale.js (currency/date formatting + bank connectors)
 *   - lib/permissions.js (tier checks for upgrade pitch + bank placeholder)
 *   - lib/budget-seed.js (default category seeding, used by lib/budget.js)
 *   - lib/merchant-categories.js (CSV note->category suggestions)
 *   - lib/budget.js (the data layer — Supabase CRUD)
 *   - lib/csv-import.js (CSV parsing engine)
 *   - lib/csv-presets.js (Desjardins/RBC/CIBC presets)
 *
 * Permissions: Budget panel HTML has zero [data-feature] attributes
 * (verified). Tier-aware rendering happens INSIDE renderers via
 * iboostPermissions.getTier(). So we don't need permissions-render.js
 * here — but we DO need lib/permissions.js for the getTier calls.
 */
(function () {
  'use strict';

  // Local escapeHtml delegate — same pattern as profile.js. The shared
  ...
  async function commitImport() {
    ...
    if (result.error) {
      // Log structured error fields so 400s aren't opaque "Object" in
      // the console. The default console.error of a PostgrestError
      // hides the message/details/hint/code that actually matter.
  ...
  async function boot() {
    if (!window.iboostAuth || !window.iboostAuth.getSessionSettled) {
      console.error('[budget] iboostAuth missing — script load order issue?');
      return;
    }
    ...
    try {
      ...
      console.error('[budget] session fetch failed:', e);
    ...
    // Run the Budget-tab init logic. This was lazy-called from
    // activateTab('budget') in the monolith; on the standalone page
    // it runs at page load (the page IS the budget tab, no need to
    // wait for activation).
    try {
      await initBudgetTab();
    } catch (e) {
      console.error('[budget] initBudgetTab failed:', e);
    }
  }
